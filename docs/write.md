...
`encode_mulaw`. `WAVEFORMATEXTENSIBLE` PCM/IEEE: `encode_extensible` /
`WavWriter::new_extensible`. No ADPCM /
G.722 / GSM encode. Channels `1..=26` (same ceiling as decode). Empty
...
| `encode_extensible` / `WavWriter::new_extensible` | `WAVEFORMATEXTENSIBLE` PCM/IEEE |
